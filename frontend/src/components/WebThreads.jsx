...
void main() {
  ...
  if (uEnableMouse > 0.5) {
    float mStr = clamp(uMouseStrength, 0.0, 1.0) * uMouseActive;
    pinchX = mix(pinchX, uMouse.x, mStr);
  ...
  float coreAmt = smoothstep(0.4, 2.0, gsum);
  col = mix(col, uColor3 * gsum, coreAmt * 0.55);
...
const WebThreads = ({
  ...
  const mouseRef = useRef({ enabled: true, strength: 0.35 });

  const activeColor3 = coreColor || color3;
  const activeMouseEnable = enableMouse && mouseInteraction;
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColor3: { value: new Float32Array(hexToRgb(activeColor3)) },
        uMouse: { value: new Float32Array([0.5, 0.5]) },
        uMouseStrength: { value: mouseStrength },
        uEnableMouse: { value: activeMouseEnable ? 1.0 : 0.0 },
  ...
  useEffect(() => {
    ...
    const rgb3 = hexToRgb(activeColor3);
    c3[0] = rgb3[0];
    c3[1] = rgb3[1];
    c3[2] = rgb3[2];
    u.uMouseStrength.value = mouseStrength;
    u.uEnableMouse.value = activeMouseEnable ? 1.0 : 0.0;
    mouseRef.current.enabled = activeMouseEnable;
  ...
  return <div ref={containerRef} className={`web-threads-container ${className}`.trim()} style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden', ...style }} />;
};
